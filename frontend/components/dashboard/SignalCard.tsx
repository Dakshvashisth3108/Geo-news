...
const directionMeta = {
  LONG:    { label: "Long",    badge: "bull",    Icon: ArrowUpRight,   tone: "text-[hsl(var(--bull))]", glow: "signal-glow-bull" },
  SHORT:   { label: "Short",   badge: "bear",    Icon: ArrowDownRight, tone: "text-[hsl(var(--bear))]", glow: "signal-glow-bear" },
  NEUTRAL: { label: "Neutral", badge: "outline", Icon: Minus,          tone: "text-muted-foreground",   glow: "" },
} as const;

/**
 * Compact representation of a TradingSignal — used both in the
 * "Recent Signals" grid and inside the LiveFeed.
 */
    ...
    <Card className={cn("relative overflow-hidden p-5 transition-shadow hover:shadow-lg", meta.glow, className)}>
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          ...
          <p className={cn("text-2xl font-semibold tabular-nums", gtiTone(gtiPct))}>
            {gtiPct}
          </p>
        </div>
      </div>
      ...
      {/* Footer: confidence + correlated assets */}
          ...
          {signal.correlated_assets.slice(0, 4).map((c) => (
            <Badge key={c.asset} variant="secondary" className="font-mono">
              ...
              <span
                className={cn(
                  "ml-1 text-[9px]",
                  c.expected_impact === "LONG" && "text-[hsl(var(--bull))]",
                  c.expected_impact === "SHORT" && "text-[hsl(var(--bear))]",
                )}
              >
                {c.correlation >= 0 ? "+" : ""}
...
function gtiTone(gti: number) {
  if (gti >= 75) return "text-[hsl(var(--bear))]";
